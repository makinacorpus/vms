...
For Vagrant you need to have a recent Vagrant version (vagrant is a virtualbox VM manager, to make it simple). But version ``1.3.4`` `is broken <https://github.com/mitchellh/vagrant/issues/2309>`_, so waiting for ``1.3.5`` you should use version ``1.3.3``. Get 
 latest vagrant from `official download site <http://downloads.vagrantup.com/>`_.
